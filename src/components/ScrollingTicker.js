...
const ScrollingTicker = () => {
  ...
  const fetchRecentPurchases = useCallback(async () => {
    try {
      setLoading(true);
      const confirmedSignatures = await connection.getConfirmedSignaturesForAddress2(tokenMintAddress, { limit: 10 });
      ...
      for (let signature of confirmedSignatures) {
        ...
        if (transaction) {
          ...
          if (solAmount > 0.05) {
            filteredPurchases.push({ signature: signature.signature, amount: solAmount });
          }
        }
      }
      ...
      setError('Unable to fetch recent purchases. Please try again later.');
    } finally {
      setLoading(false);
    }
  }, [connection, tokenMintAddress]); // Dependencies are stable
  ...
  }, [fetchRecentPurchases]); // Include the fetchRecentPurchases function as a dependency
      ...
      {loading ? (
        ...
        <ul>
          {recentPurchases.map((purchase, index) => (
            <li key={index}>
              Transaction: <span>{`${purchase.signature.slice(0, 5)}...${purchase.signature.slice(-5)}`}</span> - Amount: {purchase.amount.toFixed(2)} SOL
            </li>
          ))}
        </ul>
      )}
